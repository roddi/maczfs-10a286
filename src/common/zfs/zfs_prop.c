...
zfs_prop_init(void)
{
	...
	register_index(ZFS_PROP_READONLY, "readonly", 0, PROP_INHERIT,
	    ...
	    boolean_table);
#ifdef __APPLE__
	register_hidden(ZFS_PROP_ZONED, "zoned", PROP_TYPE_NUMBER,
	    PROP_INHERIT, ZFS_TYPE_FILESYSTEM, "ZONED");
	register_index(ZFS_PROP_XATTR, "xattr", 1, PROP_ONETIME,
	    ZFS_TYPE_FILESYSTEM | ZFS_TYPE_SNAPSHOT, "on", "XATTR",
	    boolean_table);
	register_hidden(ZFS_PROP_VSCAN, "vscan", PROP_TYPE_NUMBER,
	    PROP_INHERIT, ZFS_TYPE_FILESYSTEM, "VSCAN");
	register_hidden(ZFS_PROP_NBMAND, "nbmand", PROP_TYPE_NUMBER,
	    PROP_INHERIT, ZFS_TYPE_FILESYSTEM, "NBMAND");
#else
	...
	register_index(ZFS_PROP_CASE, "casesensitivity", ZFS_CASE_SENSITIVE,
	    PROP_ONETIME, ZFS_TYPE_FILESYSTEM | ZFS_TYPE_SNAPSHOT,
#ifdef __APPLE__
	    "sensitive | insensitive", "CASE", case_table);
#else
...
#ifdef __APPLE__
	register_hidden(ZFS_PROP_SHARENFS, "sharenfs", PROP_TYPE_STRING,
	    PROP_INHERIT, ZFS_TYPE_FILESYSTEM, "SHARENFS");
	register_hidden(ZFS_PROP_SHAREISCSI, "shareiscsi", PROP_TYPE_STRING,
	    PROP_INHERIT, ZFS_TYPE_FILESYSTEM, "SHAREISCSI");
#else
...
#ifdef __APPLE__
	register_hidden(ZFS_PROP_SHARESMB, "sharesmb", PROP_TYPE_STRING,
	    PROP_INHERIT, ZFS_TYPE_FILESYSTEM, "SHARESMB");
#else
