...
struct arc_buf_hdr {
	...
	uint32_t		b_realdatacnt; /* # buffers that store data in kernel memory instead of UPL */
#endif
...
#if defined(__APPLE_KERNEL__) && defined(ZFS_DEBUG)

LIST_HEAD(zfs_upli_hash_head, uplinfo) *upli_hash;
u_long hash_size_minus_one; //size is power of 2
kmutex_t upli_hash_mtx;
size_t num_nodes = 0;
...
//
static uint16_t hash_func(void *pv, int size);
static uint16_t
hash_func(void *pv, int size)
{
	const uint8_t *p = (const uint8_t*)pv;
	unsigned long hash = 5381;
	int c;

	ASSERT(size > 0 && p != NULL);

	while (size-- != 0) {
		c = *p++;
		if (c == 0)				/* just to make it not 0 */
			c == 77;
...
struct zfs_upli_hash_head *get_hash_entry(uplinfo_t *upli);
struct zfs_upli_hash_head *get_hash_entry(uplinfo_t *upli)
{
	u_long hash_value = hash_func(&upli, sizeof(upli)) & hash_size_minus_one;
	return upli_hash + hash_value;
}

void insert_upli(uplinfo_t *new_upli)
{
	struct zfs_upli_hash_head *hash_entry;
	uplinfo_t *upli;
	if (new_upli == NULL)
		return;
	hash_entry = get_hash_entry(new_upli);
	mutex_enter(&upli_hash_mtx);
	LIST_FOREACH(upli, hash_entry, ui_hashentry) {
		ASSERT(upli != new_upli);
	}
	LIST_INSERT_HEAD(hash_entry, new_upli, ui_hashentry);
	num_nodes++;
	mutex_exit(&upli_hash_mtx);
	debug_msg("%s upli=%p su_r=%p su_w=%p", __func__, new_upli, new_upli->ui_sharedupl_for_read, new_upli->ui_sharedupl_for_write);
}

void remove_upli(uplinfo_t *upli)
{
	mutex_enter(&upli_hash_mtx);
	LIST_REMOVE(upli, ui_hashentry);
	num_nodes--;
	mutex_exit(&upli_hash_mtx);
	debug_msg("%s upli=%p su_r=%p su_w=%p", __func__, upli, upli->ui_sharedupl_for_read, upli->ui_sharedupl_for_write);
}
#endif
...
#if defined(__APPLE_KERNEL__) && defined(ZFS_DEBUG)
	upli_hash = (struct zfs_upli_hash_head*)hashinit(1024, M_TEMP, &hash_size_minus_one);
	mutex_init(&upli_hash_mtx, NULL, MUTEX_DEFAULT, NULL);
#endif
}
...
arc_buf_destroy(arc_buf_t *buf, boolean_t recycle, boolean_t all)
{
	...
	if (buf->b_data) {
		...
		debug_msg("%s: buf=%p upli=%p supl=%p vp=%p stack: %p %p %p", __func__, buf, upli, 
				  supl, upli->ui_vp,
...
#ifdef ZFS_DEBUG
		memset(upli, 0xC1, sizeof(uplinfo_t));
#endif
		debug_msg_level(MSG_LEVEL - 10, "%s:%d free buf=%p upli=%p stack:%p %p", __func__, __LINE__, buf, upli, __builtin_return_address(1), __builtin_return_address(2));
...
#ifdef __APPLE__ /* XXX/ztest: okay to not use zfs_footprint in ztest? */
	/*
...
#ifdef __APPLE_KERNEL__
	ASSERT(hdr->b_buf->b_data != NULL && hdr->b_buf->b_uplinfo == NULL ||
		   hdr->b_buf->b_data == NULL && hdr->b_buf->b_uplinfo != NULL);
#endif
	hdr->b_acb = NULL;
...
arc_read_fill_buf(dmu_buf_impl_t *db, uplinfo_t *upli, boolean_t lock_db)
{
	...
	debug_msg("%s:%d db=%p buf=%p bupli=%p oldvp=%p upli=%p", __func__, __LINE__,
			  db, db->db_buf, db->db_buf->b_uplinfo, db->db_buf->b_uplinfo ? db->db_buf->b_uplinfo->ui_vp : NULL,
			  upli);
	if (lock_db)
...
int
#ifdef __APPLE_KERNEL__
arc_read_osx(zio_t *pio, spa_t *spa, blkptr_t *bp, arc_buf_t *pbuf,
...
#ifdef __APPLE_KERNEL__
	err = arc_read_nolock_osx(pio, spa, bp, done, private, priority,
	    flags, arc_flags, zb, upli);
#else
...
int
#ifdef __APPLE_KERNEL__
arc_read_nolock_osx(zio_t *pio, spa_t *spa, blkptr_t *bp,
    arc_done_func_t *done, void *private, int priority, int flags,
    uint32_t *arc_flags, const zbookmark_t *zb, uplinfo_t *upli)
#else
...
top:
	/* 
	 * Note: buf_hash_find will return with the hash lock held if he finds
	 * the buffer
	 */
	...
	if (hdr && hdr->b_datacnt > 0) {
		...
		*arc_flags |= ARC_CACHED;
#ifdef __APPLE_KERNEL__
		/* if this buf already has upli, it has to be the same upl as the one passed in
		ASSERT((upli==NULL || hdr->b_buf->b_uplinfo == NULL) ||
			   (hdr->b_buf->b_uplinfo->ui_vp == upli->ui_vp && 
				hdr->b_buf->b_uplinfo->ui_vid == upli->ui_vid));
		// */
		if (upli!=NULL && hdr->b_buf->b_uplinfo != NULL) {
			if (hdr->b_buf->b_uplinfo->ui_vp != upli->ui_vp ||
				hdr->b_buf->b_uplinfo->ui_vid == upli->ui_vid) {
				CODE_COVERAGE_CHECK;
				debug_msg("%s oldvp=%p vid=%d newvp=%p vid=%d refcnt=%d", __func__,
						  hdr->b_buf->b_uplinfo->ui_vp, (int)hdr->b_buf->b_uplinfo->ui_vid,
						  upli->ui_vp, (int)upli->ui_vid, (int)refcount_count(&hdr->b_refcnt));
				ASSERT(hdr->b_state != arc_anon);
				ASSERT(refcount_is_zero(&hdr->b_refcnt));
				arc_change_state(arc_anon, hdr, hash_lock);
				arc_hdr_destroy(hdr);
...
#ifdef __APPLE_KERNEL__
			/* we need to copy data to upl when we have upli.  So if upli is not NULL, force to wait */
			if ((*arc_flags & ARC_WAIT) || (upli != NULL))
#else
...
#ifdef __APPLE_KERNEL__
			/* XXX Prefetching at UBC level not implemented yet.  done is NULL only when prefetching (where upli must be
			 * NULL), so no need to do anything here before prefetching at UBC level is implemented
			 */
			ASSERT(upli == NULL);
#endif
			mutex_exit(hash_lock);
		...
		if (done) {
			...
			ASSERT(buf);
#ifdef __APPLE_KERNEL__
			ASSERT(buf->b_data || buf->b_uplinfo);
#else
			ASSERT(buf->b_data);
#endif
		...
		if (buf->b_data && upli) {
			CODE_COVERAGE_CHECK;
			sharedupl_t *supl = upli_sharedupl(upli);
			VERIFY(copy_mem_to_upl(supl->su_upl, upli->ui_f_off - supl->su_upl_f_off,
								   buf->b_data, hdr->b_size, supl->su_pl) == 0);
		}
#endif
...
#ifdef __APPLE_KERNEL__
			buf = arc_buf_alloc(spa, size, private, type, alloc_data);
			if (!alloc_data) {
				CODE_COVERAGE_CHECK;
				buf->b_uplinfo = upli;
				insert_upli(upli);
				debug_msg("%s:%d created buf %p upli=%p stack: %p %p %p %p %p %p", __func__, __LINE__, buf, upli,
...
#ifdef __APPLE_KERNEL__
		rzio = zio_read_osx(pio, spa, bp, buf->b_data, size,
			arc_read_done, buf, priority, flags, zb, upli);
#else
...
top:
	...
	if (hdr && hdr->b_datacnt > 0 && !HDR_IO_IN_PROGRESS(hdr)) {
		...
		ASSERT(buf);
#ifdef __APPLE_KERNEL__
		while (buf->b_data == NULL && buf->b_uplinfo == NULL)
#else
		...
		if (buf->b_uplinfo != NULL && buf->b_data == NULL) {
			...
			if (vnode_vid(upli->ui_vp) != upli->ui_vid) {
				/* this buffer is already a stale one, remove it from ARC and check again */
				ASSERT(hdr->b_state != arc_anon);
				ASSERT(refcount_is_zero(&hdr->b_refcnt));
				arc_change_state(arc_anon, hdr, hash_mtx);
				arc_hdr_destroy(hdr);
				mutex_exit(hash_mtx);
				goto top;
			}
			/* map UPL into kernel (if not mapped yet) for the checksum */
			if (upli_sharedupl(buf->b_uplinfo) == NULL)
				need_unmap = TRUE;
			p = getuplvaddr(buf->b_uplinfo, FALSE/*for_read*/) +
				(buf->b_uplinfo->ui_f_off - upli_sharedupl(buf->b_uplinfo)->su_upl_f_off);
		} else {
		...
		bcopy(p, data, hdr->b_size);
		if (need_unmap) {
			CODE_COVERAGE_CHECK;
			sharedupl_put(buf->b_uplinfo, FALSE/*clear_dirty*/);
		}
#else
...
/* 
 * UBC integration on Mac OSX:
 * Only decrement the size if the data actually was being stored in the arc_buf_t
 */
#ifdef __APPLE_KERNEL__
		if (buf->b_data != NULL)
#endif
...
#ifdef __APPLE_KERNEL__
		if (buf->b_data != NULL)
			nhdr->b_realdatacnt = 1;
		else
			nhdr->b_realdatacnt = 0;
#endif
		nhdr->b_freeze_cksum = NULL;
		(void) refcount_add(&nhdr->b_refcnt, tag);
		buf->b_hdr = nhdr;
/* 
 * UBC integration on Mac OSX: the data may not be stored in the arc_buf_t, 
 * it may be in the vm.  So don't add the blksz if the data isn't being stored in the ARC.
 */
#ifdef __APPLE_KERNEL__
		if (buf->b_data != NULL)
#endif
	...
	if (l2hdr) {
		...
		ARCSTAT_INCR(arcstat_l2_size, -bufsize);
	}
	if (MUTEX_HELD(&l2arc_buflist_mtx))
		mutex_exit(&l2arc_buflist_mtx);
}
...
{
#ifdef __APPLE_KERNEL__
	return ((buf->b_data != NULL || buf->b_uplinfo != NULL) &&
	        (buf->b_hdr->b_state == arc_anon));
#else
...
#ifdef __APPLE_KERNEL__
	if (zio->io_uplinfo) {
		sharedupl_put(zio->io_uplinfo, TRUE/*clear_dirty*/);
	}
#endif
    ...
    int flags, const zbookmark_t *zb)
{
	...
	if (buf->b_uplinfo) {
		...
		sharedupl_t *supl;
		off_t upl_start_off;
		sharedupl_get(buf->b_uplinfo, FALSE/*for_read*/);
		upli = buf->b_uplinfo;
		supl = upli_sharedupl(upli);
		debug_msg("%s buf=%p upli=%p vp=%p vid=%d", __func__, buf,
		    buf->b_uplinfo, buf->b_uplinfo->ui_vp,
		    (int)vnode_vid(buf->b_uplinfo->ui_vp));

		/*
		 * check whether the upl that covers this block has hole.  if it does, we have to do read-modify-write instead
		 */
		boolean_t has_hole = FALSE;
		int page_index, page_index_start, num_pages;
		upl_start_off = upli->ui_f_off - supl->su_upl_f_off;
		page_index_start = upl_start_off / PAGE_SIZE;
		num_pages = howmany(hdr->b_size, PAGE_SIZE);
		CODE_COVERAGE_CHECK;
		if (supl->su_pl) {
			CODE_COVERAGE_CHECK;
			for (page_index = 0; page_index < num_pages; page_index++) {
				if (!upl_valid_page(supl->su_pl, page_index + page_index_start)) {
					has_hole = TRUE;
					debug_msg("%s upl=%p invalid page=%d", __func__, supl->su_upl, page_index + page_index_start);
					break;
				}
			}
		}
		if (has_hole) { /* do read modify write here */
			/* convert the buffer to use kernel memory instead of VM */
			buf->b_uplinfo = NULL;
			arc_get_data_buf(buf);
			arc_buf_hdr_set_realdatacnt(buf->b_hdr, arc_buf_hdr_realdatacnt(buf->b_hdr) + 1);

			/* read data from media so the data not covered by this upl will have valid data */
			if (bp == NULL || BP_IS_HOLE(bp)) {
				CODE_COVERAGE_CHECK;
				bzero(buf->b_data, buf->b_hdr->b_size);
			} else {
				CODE_COVERAGE_CHECK;
				buf->b_hdr->b_flags |= ARC_IO_IN_PROGRESS;
				zio = zio_read_osx(NULL/*pio*/, spa, bp, buf->b_data,
								   buf->b_hdr->b_size, arc_read_fill_buf_done, buf, ZIO_PRIORITY_SYNC_READ,
								   ZIO_FLAG_CANFAIL, zb, NULL/*upli*/);
			...
			bytes_left = hdr->b_size;
			for (page_index = 0; page_index < num_pages; page_index++) {
				bytes_to_copy = MIN(bytes_left, PAGE_SIZE);
				if (upl_valid_page(supl->su_pl, page_index + page_index_start)) {
					copy_upl_to_mem(supl->su_upl, upl_start_off + page_index * PAGE_SIZE,
									buf->b_data + page_index * PAGE_SIZE, bytes_to_copy, supl->su_pl);
				}
				bytes_left -= bytes_to_copy;
			}
			sharedupl_put(upli, TRUE/*clear_dirty*/);
		} /* if (has_hole) */
	} /* if (buf->b_uplinfo) */
	zio = zio_write(pio, spa, cksum, comp, copies, txg, bp,
	    buf->b_data, hdr->b_size, arc_write_ready, arc_write_done,
	    callback, priority, flags, zb, buf->b_uplinfo);
#else
	zio = zio_write(pio, spa, cksum, comp, copies, txg, bp,
	    buf->b_data, hdr->b_size, arc_write_ready, arc_write_done,
...
arc_init(void)
{
#ifdef __APPLE__ /* XXX/zest: should these numbers be greater for ztest? */
	/*
	 * Use more conservative limits in Mac OS X
	 *
	 * 2/3 of maximum zfs footprint
	 */
	zfs_arc_max = 10 * 1048576; // (zfs_footprint.maximum / 3) * 2;
	zfs_arc_min = 2 * 1048576; //MAX((physmem * PAGESIZE) / 16, 64<<20);
#endif
...
#if defined(__APPLE_KERNEL__) && defined(ZFS_DEBUG)
	FREE(upli_hash, M_TEMP);
	mutex_destroy(&upli_hash_mtx);
#endif
	buf_fini();
}
