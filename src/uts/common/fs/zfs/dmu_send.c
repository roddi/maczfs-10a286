...
#ifdef __APPLE__
#include <sys/zfs_file.h>	/* for fd_rdwr prototype */
#endif
...
{
#ifdef __APPLE__
	int64_t resid; /* have to get resid to get detailed errno */
#else
	 ...
	 */
	ba->err = fd_rdwr(ba->fd, UIO_WRITE, (uint64_t)(u_long)buf, (int64_t)len, 
	    UIO_SYSSPACE, (off_t)0, IO_APPEND, &resid);
#else
...
restore_read(struct restorearg *ra, int len)
{
	...
	while (done < len) {
		...
		ra->err = fd_rdwr(ra->fd, UIO_READ,
		    (uint64_t)((u_long)ra->buf + done),
		    (int64_t)((int64_t)len - done), UIO_SYSSPACE,
		    (off_t)ra->voff, IO_APPEND, &resid);
#else
		ssize_t resid;
