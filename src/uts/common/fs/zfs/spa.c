...
#ifdef __APPLE__
		    spa->spa_spares.sav_vdevs[i], B_TRUE, B_TRUE, B_FALSE, B_FALSE);
#else
...
#ifdef __APPLE__
		    sav->sav_vdevs[i], B_TRUE, B_FALSE, B_TRUE, B_FALSE);
#else
...
#ifdef __APPLE__
	/* allows zdb -p -e to work; fixed in more current Solaris */
    if (error != 0 || (props && (spa_mode & FWRITE) &&
...
spa_export_common(char *pool, int new_state, nvlist_t **oldconfig)
{
	...
	if (new_state != POOL_STATE_UNINITIALIZED) {
#ifdef __APPLE_KERNEL__
		zfs_iomedia_terminate(spa_guid(spa));
#endif
		spa_remove(spa);
	}
	mutex_exit(&spa_namespace_lock);
...
spa_sync_props(void *arg1, void *arg2, cred_t *cr, dmu_tx_t *tx)
{
	...
	while ((elem = nvlist_next_nvpair(nvp, elem))) {
		...
		default:
			...
			mutex_enter(&spa->spa_props_lock);
			if (spa->spa_pool_props_object == 0) {
				objset_t *_mos = spa->spa_meta_objset;

				VERIFY((spa->spa_pool_props_object =
				    zap_create(_mos, DMU_OT_POOL_PROPS,
				    DMU_OT_NONE, 0, tx)) > 0);

				VERIFY(zap_update(_mos,
