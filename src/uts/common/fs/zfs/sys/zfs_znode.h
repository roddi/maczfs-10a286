...
/* NOTE: ZFS_BONUS_SCANSTAMP and ZFS_BONUS_FINDERINFO are mutually exclusive */
#define	ZFS_BONUS_SCANSTAMP	0x080		/* Scanstamp in bonus area */
#define	ZFS_BONUS_FINDERINFO	0x800		/* Finder Info in bonus area */
...
typedef struct znode_phys {
	 ...
	 *  - 32-byte OS X Finder Info (directories and regular files only)
	 */
} znode_phys_t;
...
#ifdef __APPLE__
struct wextent {
	daddr64_t	b_addr; // first logical page number
	daddr64_t	e_addr; // last logical page number
};
/* start to push whenever have MAX_CLUSTERS clusters */
#define MAX_CLUSTERS 4
/* follow the VFS policy, push out extents when reached 1MB */
#define MAX_EXTENT_SIZE MAX_UPL_SIZE
#endif
...
typedef struct znode {
...
#ifdef __APPLE__
	uint8_t		z_mmapped_for_write;	/* file has been memory mapped 
						 * for write */
	...
	uint32_t	z_vid;		/* id (gen) of associated vnode */
...
#ifdef __APPLE__
extern int zfs_cluster_setsize(znode_t *zp, off_t size);
extern int extent_cmp(const void *p1, const void *p2);
#endif
...
#ifdef __APPLE__
extern int zfs_read(vnode_t *vp, struct uio *uio);
