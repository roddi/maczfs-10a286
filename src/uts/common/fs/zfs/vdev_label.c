...
#ifdef __APPLE__
    boolean_t isspare, boolean_t isl2cache, boolean_t skipdevpaths)
#else
...
#ifdef __APPLE__
	/*
	 * On OS X we only write the device path if its a file
	 * vdev or this config is not going to a disk label.
	 */
	if ((vd->vdev_path != NULL) &&
	    (strcmp(vd->vdev_ops->vdev_op_type, VDEV_TYPE_FILE) == 0 ||
	     skipdevpaths == B_FALSE))
#else
	if (vd->vdev_path != NULL)
#endif
...
#ifdef __APPLE__
			    getstats, isspare, isl2cache, skipdevpaths);
#else
