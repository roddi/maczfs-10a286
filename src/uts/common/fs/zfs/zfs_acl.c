    ...
    zfs_acl_t *setaclp, zfs_fuid_info_t **fuidp)
{
	...
	zfs_acl_t	*paclp;
#ifndef __APPLE__
	/* OS X - we don't utilize any extended vnode attributes */
...
#ifndef __APPLE__
	/* OS X - access checking occurs in VFS layer */
...
zfs_setacl(znode_t *zp, vsecattr_t *vsecp, boolean_t skipaclchk, cred_t *cr)
{
	...
	error = zfs_vsec_2_aclp(zfsvfs, vnode_vtype(ZTOV(zp)), vsecp, &aclp);
#else
