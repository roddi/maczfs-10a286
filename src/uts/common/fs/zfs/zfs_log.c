...
#ifdef __APPLE__
/* 
 * was 32768; changed for ARC UBC bug (6011160) force zil_commit to use WR_INDIRECT so that no
 * dirty buffers of the transaction exist in ARC after zil_commit
 */
ssize_t zfs_immediate_write_sz = 0;
#else
	...
	znode_t *zp, offset_t off, ssize_t resid, int ioflag)
{
	 ...
	 *    transaction (FSYNC or FDSYNC (O_DSYNC)), the we allocate a larger
