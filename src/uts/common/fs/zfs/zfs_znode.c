...
#ifdef __APPLE__
	/*
	 * During a forced unmount we can end up
	 * here to clean up any orphaned znodes
	 */
	znode_t *zp = user_ptr;

	if (zp && zp->z_zfsvfs && POINTER_IS_VALID(zp->z_zfsvfs) &&
	    vfs_isforce(zp->z_zfsvfs->z_vfs)) {
		zfs_znode_free(zp);
		return;
	}
#endif /* __APPLE__ */
...
#ifdef __APPLE__
	cv_destroy(&zp->z_cv);
	ASSERT(avl_numnodes(&zp->z_upltree) == 0);
...
#ifndef __APPLE__
	/* OS X doesn't yet support movable slab objects */
...
#ifdef __APPLE__
	/* keep evict function around during a forced unmount */
	if (!vfs_isforce(zp->z_zfsvfs->z_vfs))
#endif
 ...
 */
static int
zfs_zget_internal(zfsvfs_t *zfsvfs, uint64_t obj_num, znode_t **zpp, int skip_vnode, int want_unlinked)
{
...
again:
	...
	if (doi.doi_bonus_type != DMU_OT_ZNODE ||
		...
		dmu_buf_rele(db, NULL);
		ZFS_OBJ_HOLD_EXIT(zfsvfs, obj_num);
		debug_msg("%s:%d will return EINVAL", __func__, __LINE__);
		return (EINVAL);
	}
...
top:
	 ...
	 */
#ifndef __APPLE__
	/* OS X - This work is accomplished by ubc_setsize() */
